...
public class SLList {
    ...
    public void add(int x, int index) {
        // TODO
        size++;
        IntListNode res=sentinel;
        for(int i=0;i<x&&res.next!=sentinel;++i)
        {
            res = res.next;
        }
        if(res.next==null)
        {
            IntListNode n=new IntListNode(index,null);
            res.next=n;
            return;
        }
        IntListNode ne=res.next;
        IntListNode n=new IntListNode(index,ne);
        res.next=n;
    }
    ...
    public void reverse() {
        ...
        int i=0;
        IntListNode res=sentinel.next;
        while(res!=sentinel)
        {
            a[i++]=res.item;
            res=res.next;
        }
        for(int ii=0,j=size-1;ii<=j;++ii,--j)
        {
            int tmp=a[ii];
            a[ii]=a[j];
            a[j]=tmp;
